...
build_plot_labels <- function(metric_name, interval, repo_names, aggregation = NULL, chart_type = NULL) {
  ...
  repo_label <- if (length(repo_names) == 1 && identical(repo_names, "ALL")) {
    "ALL"
  } else if (length(repo_names) > 3) {
    if (lang == "ja") {
      sprintf("%s など %d 件", repo_names[1], length(repo_names) - 1)
    } else {
      sprintf("%s and %d more", repo_names[1], length(repo_names) - 1)
    }
  } else {
...
plot_metric_trend <- function(metrics,
                              ...
                              y_label = NULL,
                              y_limits = NULL,
                              chart_type = c("line", "stacked_with_line"),
                              label_repos = NULL) {
  aggregation <- match.arg(aggregation)
  chart_type <- match.arg(chart_type)
  repo_names_for_label <- label_repos
  ...
  if (chart_type == "stacked_with_line") {
    ...
    if (nrow(filtered_bars) == 0 && nrow(filtered_line) == 0) {
      cli::cli_warn("[{metric_name}] 可視化対象データが存在しないため、グラフをスキップします。")
      return(invisible(NULL))
    }

    filtered_bars <- filtered_bars %>%
      dplyr::arrange(period_start, actor_type, repo) %>%
    ...
    aggregated_bars <- switch(
      aggregation,
      sum = filtered_bars %>%
        dplyr::group_by(period_start, actor_type, repo) %>%
  ...
  if (chart_type == "stacked_with_line") {
    value_unit <- filtered_line$value_unit[which(nzchar(filtered_line$value_unit))[1]] %||% filtered_line$value_unit[1] %||% ""
  } else {
  ...
  if (chart_type == "stacked_with_line") {
    upper <- suppressWarnings(max(aggregated_line$metric_value, na.rm = TRUE))
  ...
  if (chart_type == "stacked_with_line") {
    y_max <- suppressWarnings(max(aggregated_line$metric_value, na.rm = TRUE))

    g <- ggplot() +
      geom_col(
        data = aggregated_bars,
        aes(x = period_start, y = metric_value, fill = repo),
        position = "stack",
        width = bar_width,
        alpha = 0.7
      ) +
      geom_line(
        data = aggregated_line,
        aes(x = period_start, y = metric_value, color = actor_type),
        linewidth = 1.2
      ) +
      geom_point(
        data = aggregated_line,
      ...
      scale_fill_brewer(palette = "Set2", name = "Repository") +
      scale_color_manual(
        values = c(human = "#E74C3C", agent = "#3498DB"),
        name = labels$color_label
      ) +
      scale_x_date(
        date_labels = "%Y-%m",
